...
const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      localStorage.setItem('sarthi_vol_id', data.user_id.toString())
      localStorage.setItem('sarthi_vol_name', data.name)
      navigate('/dashboard')
    } catch (err) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {error && (
          <div style={{ padding: '0.75rem', background: 'rgba(231,76,60,0.15)', border: '1px solid rgba(231,76,60,0.4)', color: '#e74c3c', borderRadius: '12px', marginBottom: '1.25rem', fontSize: '0.85rem', textAlign: 'center' }}>
            {error}
          </div>
        )}
        ...
        <div style={{ textAlign: 'right', marginBottom: '1.75rem' }}>
          <a href="/forgot-password" onClick={(e) => { e.preventDefault(); navigate('/forgot-password'); }} style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', textDecoration: 'none' }}
