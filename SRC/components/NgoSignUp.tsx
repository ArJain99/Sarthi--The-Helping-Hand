    ...
    try {
      const resp = await fetch('http://localhost:8000/api/profiles/', {
        ...
        body: JSON.stringify({
          ...
          email: form.email,
          role: 'NGO',
          bio: `Type: ${form.orgType}, RegNo: ${form.regNumber}, Contact: ${form.contactName}, Phone: ${form.phone}`,
          skills_or_needs: `Website: ${form.website}, Address: ${form.address}, ${form.city}, ${form.state}, ${form.country}. Mission: ${form.mission}`,
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #1a2e44 0%, #1e3a5f 45%, #22406e 100%)', padding: '88px 1rem 3rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto' }}>
        ...
        {errors.general && (
          <div style={{ background: 'rgba(231,76,60,0.15)', border: '1px solid rgba(231,76,60,0.4)', color: '#fc8a8a', padding: '1rem', borderRadius: '12px', textAlign: 'center', fontSize: '0.95rem', fontWeight: '600', marginBottom: '1.5rem' }}>
            {errors.general}
          </div>
        )}
