...
const NgoLogin = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('sarthi_ngo_id', data.user_id.toString())
      localStorage.setItem('sarthi_ngo_email', data.name)
    ...
    <div style={{
      ...
      <div style={{ background: 'rgba(255,255,255,0.06)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '24px', padding: '2.5rem', width: '100%', maxWidth: '420px', boxShadow: '0 20px 60px rgba(0,0,0,0.4)' }}>
        ...
        <div style={{ textAlign: 'right', marginBottom: '1.75rem' }}>
          <a href="/forgot-password" onClick={(e) => { e.preventDefault(); navigate('/forgot-password'); }} style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.85rem', textDecoration: 'none' }}
