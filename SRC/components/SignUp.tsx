...
const SignUp = () => {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    conduct: false, terms: false, password: ''
  })
    ...
    try {
      const resp = await fetch('http://localhost:8000/api/profiles/', {
        ...
        body: JSON.stringify({
          ...
          email: form.email,
          role: 'Volunteer',
          bio: `DOB: ${form.dob}, Gender: ${form.gender}, Phone: ${form.phone}`,
          skills_or_needs: `Experience: ${form.experience}, Skills: ${form.skills}, Interests: ${form.interests.join(', ')}`,
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #1abc9c 0%, #16a085 40%, #148f77 70%, #1a7a6e 100%)', padding: '80px 1rem 3rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto' }}>
        ...
        {errors.general && (
          <div style={{ background: 'rgba(231,76,60,0.15)', border: '1px solid rgba(231,76,60,0.4)', color: '#e74c3c', padding: '1rem', borderRadius: '12px', textAlign: 'center', fontSize: '0.95rem', fontWeight: '600', marginBottom: '1.5rem' }}>
            {errors.general}
          </div>
        )}
          ...
          <Section title="Personal Information">
            ...
            <div style={rowStyle}>
              <Field label="Password" required error={errors.password}>
                <input type="password" name="password" value={form.password} onChange={handleInput} placeholder="Minimum 6 characters" style={inputStyle(!!errors.password)} />
              </Field>
              <div />
            </div>
          </Section>
